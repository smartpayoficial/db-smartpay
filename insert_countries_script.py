...
def main():
    ...
    uuid_command = 'docker exec -i smartpay-db-prod psql -U postgres -d smartpay_prod_db -c "CREATE EXTENSION IF NOT EXISTS \"uuid-ossp\";"'
    ...
    sql_command = f'docker exec -i smartpay-db-prod psql -U postgres -d smartpay_prod_db < {sql_script}'
    ...
    verify_command = 'docker exec -i smartpay-db-prod psql -U postgres -d smartpay_prod_db -c "SELECT COUNT(*) FROM country; SELECT COUNT(*) FROM region; SELECT COUNT(*) FROM city;"'
