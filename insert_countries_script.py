...
def main():
    ...
    uuid_command = 'docker exec -i docker-smartpay-db-1 psql -U postgres -d smartpay -c "CREATE EXTENSION IF NOT EXISTS \\"uuid-ossp\\";";'
    ...
    sql_command = f'docker exec -i docker-smartpay-db-1 psql -U postgres -d smartpay < {sql_script}'
    ...
    verify_command = 'docker exec -i docker-smartpay-db-1 psql -U postgres -d smartpay -c "SELECT COUNT(*) FROM country; SELECT COUNT(*) FROM region; SELECT COUNT(*) FROM city;"'
