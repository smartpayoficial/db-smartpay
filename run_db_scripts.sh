...
run_sql_command() {
    ...
    if docker exec -i docker-smartpay-db-1 psql -U postgres -d smartpay_dev_db -c "$sql_command"; then
        print_message $GREEN "✓ $description completado exitosamente"
    else
        print_message $RED "✗ Error al ejecutar $description"
        exit 1
...
main() {
    ...
    if docker exec -i docker-smartpay-db-1 psql -U postgres -d smartpay_dev_db -c "SELECT COUNT(*) FROM store;" 2>/dev/null; then
