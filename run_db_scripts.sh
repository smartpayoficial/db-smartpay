...
check_db_container() {
    if ! docker ps | grep -q "smartpay-db-prod"; then
        print_message $RED "Error: El contenedor de la base de datos (smartpay-db-prod) no está ejecutándose."
...
run_sql_command() {
    ...
    if docker exec -i smartpay-db-prod psql -U postgres -d smartpay_prod_db -c "$sql_command"; then
        print_message $GREEN "✓ $description completado exitosamente"
    else
        print_message $RED "✗ Error al ejecutar $description"
        exit 1
...
main() {
    ...
    if docker exec -i smartpay-db-prod psql -U postgres -d smartpay_prod_db -c "SELECT COUNT(*) FROM store;" 2>/dev/null; then
